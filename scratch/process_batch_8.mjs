...
filesToCopy.forEach(m => {
  ...
  results.slice(0, 8).forEach((r, idx) => {
    const line = `  Match #${idx + 1}: ${r.name} (${r.width}x${r.height}) -> Distance: ${r.distance.toFixed(2)}\n`;
...
fs.writeFileSync('scratch/ascii_art_batch_8_results.txt', outputText);
console.log('\nSaved ASCII art results to scratch/ascii_art_batch_8_results.txt');
