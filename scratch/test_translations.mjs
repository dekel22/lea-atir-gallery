...
function translateCaption(caption) {
  ...
  const translatedParts = parts.map(part => {
    ...
    let temp = p;
    temp = temp.replace('טשע תמונות, כל אחת', 'Nine pictures, each'); // Fix Hebrew typo in text
...
console.log("=== TRANSLATION PREVIEWS ===");
fullCaptions.forEach(c => {
  console.log(`HE: ${c}`);
  console.log(`EN: ${translateCaption(c)}`);
  console.log("---");
});
