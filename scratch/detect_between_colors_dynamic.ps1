...
foreach ($d in $allDirs) {
    # Match directories containing 'ב' (0x05D1) and 'ל' (0x05DC)
    if ($d.Name.Contains([char]0x05D1) -and $d.Name.Contains([char]0x05DC)) {
        $targetDir = $d
        break
    }
}
...
Write-Output "Scanning directory: $($targetDir.FullName)"
